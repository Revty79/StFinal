...
type CharacterTabKey = "identity" | "attributes" | "skills" | "story" | "connections" | "preview";
...
export default function CharacterBuilderPage() {
  ...
  useEffect(() => {
    function loadCharacters() {
      try {
        const stored = localStorage.getItem('serrian_characters');
        if (stored) {
          const parsed = JSON.parse(stored);
          setCharacters(parsed);
        }
        
        // TODO: Load races and skills from API or localStorage
        // For now, using empty arrays - will populate later
        setRaces([]);
        setAllSkills([]);
      } catch (error) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs sm:text-sm text-zinc-300/90 max-w-2xl">
              Create your player character for Serrian Tide campaigns.
              Build your identity, allocate attributes and skills, craft your backstory,
      ...
      <section className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[300px,1fr] gap-4 sm:gap-6 mt-4 sm:mt-6">
        ...
        >
          ...
          <div className="mt-2 flex-1 min-h-0 overflow-hidden rounded-2xl border border-white/10 bg-black/30">
            <div className="flex items-center justify-between px-3 py-2 text-xs text-zinc-400 border-b border-white/10">
              <span>Characters: {filteredList.length}</span>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs min-w-[300px]">
                    <thead className="text-left text-zinc-400">
                      <tr>
                        <th className="px-3 py-1">Character Name</th>
                        <th className="px-3 py-1">Player</th>
                        <th className="px-3 py-1">Race</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredList.map((n) => {
                          ...
                          >
                            <td className="px-3 py-1.5">
                              <div className="flex items-center gap-2">
                                <span>{n.characterName || "(unnamed)"}</span>
                              </div>
                            </td>
                            <td className="px-3 py-1.5">
                              {n.playerName ?? "—"}
            ...
            <div className="flex items-center justify-between px-3 py-2 text-xs text-zinc-400 border-t border-white/10">
              <div className="flex flex-col gap-1 flex-1 pr-2">
                ...
                <input
                  ...
                  onChange={(e) => updateSelected({ characterName: e.target.value })}
                />
              </div>
              <Button
                variant="secondary"
        ...
        >
          {loading ? (
            ...
            <p className="text-sm text-zinc-400">
              Select a character on the left or create a new one to begin editing.
            </p>
          ) : (
            <>
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <FormField
                        label="Race"
                        htmlFor="char-race"
                        description="Will be a dropdown when DB is connected"
                      >
                        <Input
                          id="char-race"
                          value={selected.race ?? ""}
                ...
                {activeTab !== "identity" && activeTab !== "attributes" && activeTab !== "skills" && activeTab !== "story" && (
                  <Card className="rounded-2xl border border-blue-300/30 bg-blue-300/5 p-4">
